...
server.post("/devices/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.title); // Generate a slug for new devices.
    next();
  }
});
...
function validatedevice(device) {
  if (!device.name) return "Title is required.";
  if (!device.category) return "Category is required.";
  return "";
}
